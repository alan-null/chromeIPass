---
layout: default
title: 6. Limitations and known problems
nav_order: 6
---
...
{: .warning }
Because Google Chrome does not offer an API to communicate with the built-in password manager, chromeIPass needs to implement its own kind of password manager. Please read the known limitations below.

Note: for security reasons chromeIPass wipes out the temporarily stored credentials from cache every time you switch tabs. Therefore it requests credentials for the current tab every time you switch to it.

### 6.1 Remember credentials
...
### 6.2 Auto detection of input fields
